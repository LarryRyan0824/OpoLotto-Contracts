...
contract OpoLotto is Owned {
    ...
    uint16 constant minimumPlayers = 10;
    uint48 constant secondsUntilDrawIsOpen = 108000;
    ...
    uint256 constant minimumToPlay = 100 trx;
    uint256 constant minimumToDraw = 5000 trx; 
    ...
    // Modifiers
    modifier validValue() {
        require(msg.value >= minimumToPlay, "Minimum amount to play is not reached.");
        _;
    }

    modifier validRoundNumber(uint256 _roundNumber) {
        require(_roundNumber == currentRoundNumber, "You are too late for this round.");
        _;
    }
    
    modifier validToDraw() {
        require(
            totalPrizeThisRound >= minimumToDraw && numberOfPlayersThisRound >= minimumPlayers,
    ...
    modifier validToDrawTime() {
        require(block.timestamp > lastDrawTime + secondsUntilDrawIsOpen, "Too early to draw.");
        _;
    }

    modifier isPlayerInThisRound() {
        uint48 index = playersIndex[msg.sender];
        require(index > 0 && index <= numberOfPlayersThisRound && players[index].playerAddress == msg.sender, "Player not eligible to draw.");
        _;
    }
    ...
    */
    function enter(uint256 _roundNumber) public payable validValue validRoundNumber(_roundNumber) returns (string memory) {
        totalPrizeThisRound += msg.value;
        uint48 index = playersIndex[msg.sender];

        if (index > numberOfPlayersThisRound || index == 0 || players[index].playerAddress != msg.sender) {
            numberOfPlayersThisRound++;
            players[numberOfPlayersThisRound] = Player(msg.sender, msg.value);
            playersIndex[msg.sender] = numberOfPlayersThisRound;
    ...
    */
    function draw() public payable validToDraw validToDrawTime isPlayerInThisRound returns (address payable[] memory) {
        ...
        players[0].playerAddress.transfer((totalPrizeThisRound * earlybirdRewardShare) / 1000);
        players[1].playerAddress.transfer((totalPrizeThisRound * earlybirdRewardShare) / 1000);
        players[2].playerAddress.transfer((totalPrizeThisRound * earlybirdRewardShare) / 1000);
        players[3].playerAddress.transfer((totalPrizeThisRound * earlybirdRewardShare) / 1000);
        players[4].playerAddress.transfer((totalPrizeThisRound * earlybirdRewardShare) / 1000);
        ...
        previousGameResults[currentRoundNumber].roundNumber = currentRoundNumber;
        previousGameResults[currentRoundNumber].totalPrize = totalPrizeThisRound;
        previousGameResults[currentRoundNumber].numberOfPlayers = numberOfPlayersThisRound;
    ...
    */
    ...
    {
        require(roundNum > 0 && roundNum < currentRoundNumber, "Invalid round number.");
    ...
    */
    function playerList() public view returns (address[] memory _addresses, uint256[] memory _values) {
    ...
    */
    function playerTotal(address _address) public view returns (uint256) {
        ...
        if (index > numberOfPlayersThisRound || index == 0 || players[index].playerAddress != _address) {
            return 0;
        }
    ...
    function remainTime() private view returns (uint256) {
        int256 time = int256(secondsUntilDrawIsOpen + lastDrawTime - block.timestamp);
        if (time < 0) return 0;
        else return uint256(time);
    }

    function random(uint256 max, uint8 randomnessType) private view returns (uint256) {
    ...
    function earlyBirdList() private view returns (address[] memory _addresses) {
        if (numberOfPlayersThisRound == 0) {
            return _addresses;
        }
